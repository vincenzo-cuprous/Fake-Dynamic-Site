    ...
    function handleBackup() {
        const backupData = {
            adminCredentials: {
                username: localStorage.getItem('adminUsername') || adminCredentials.username,
                password: localStorage.getItem('adminPassword') || adminCredentials.password
            },
            books: books,
            comments: comments
        };
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        localStorage.setItem('adminUsername', restoredData.adminCredentials.username);
                        localStorage.setItem('adminPassword', restoredData.adminCredentials.password);
                        localStorage.setItem('books', JSON.stringify(restoredData.books));
                        localStorage.setItem('comments', JSON.stringify(restoredData.comments));
                        
                        // Update the current data
                        adminCredentials.username = restoredData.adminCredentials.username;
                        adminCredentials.password = restoredData.adminCredentials.password;
                        books = restoredData.books;
                        comments = restoredData.comments;
                        
                        // Refresh the books table and comments
                        loadBooksTable();
                        displayBooks(books);
                        if (currentBookId) {
                            displayComments(currentBookId);
